...
var onClientsDone = function(list){
  async.mapLimit(list, 10, function(obj, callback) {
    ...
    if (err) {
      ...
      var dataVtexDay = new Date("2016-08-22 11:37:37.504577");
      var newCients = _.filter(results, function (clt) {
        var dataClient = new Date(clt.data_criacao);
        return dataClient >= dataVtexDay;
      });
      console.log(newCients.length);

      for (var i = 0; i < newCients.length; i++) {
        var client = new Client(newCients[i]);
        client.addresses = [];
        _.each(newCients[i].enderecos, function(add){
          client.addresses.push(new Address(add));
        });
        clientListDetailed.push(client);
      }
      console.log(clientListDetailed.length);
      console.log(clientListDetailed[1]);
      vtexClientWriter.createClients(clientListDetailed)
    }
  });
};
